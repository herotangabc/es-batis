...
const error = (...err: any) => { throw Error(...err) }

// Parse SQL statement from provided arguments
const parseSQL = (args: any) =>
...
const prepareParams = ({ secretArn, resourceArn }: { secretArn: string, resourceArn: string }, args: any) => {
...
// Prepare parameters
const processParams = (engine: any, sql: string, sqlParams: any, params: any, formatOptions: any, row = 0) => {
  return {
    processedParams: params.reduce((acc: any, p: any) => {
      if (Array.isArray(p)) {
...
const formatParam = (n: any, v: unknown, formatOptions: any) => formatType(n, v, getType(v as any), getTypeHint(v), formatOptions)

// Converts object params into name/value format
const splitParams = (p: any) => Object.keys(p).reduce((arr: any, x) =>
...
const getType = (val: unknown) =>
  typeof val === 'string' ? 'stringValue'
    : typeof val === 'boolean' ? 'booleanValue'
      : typeof val === 'number' && parseInt(val as unknown as string) === val ? 'longValue'
        : typeof val === 'number' && parseFloat(val as unknown as string) === val ? 'doubleValue'
          : val === null ? 'isNull'
            : isDate(val) ? 'stringValue'
              : Buffer.isBuffer(val) ? 'blobValue'
                ...
                : typeof val === 'object'
                  && Object.keys(val as unknown as any).length === 1
                  && supportedTypes.includes(Object.keys(val as unknown as any)[0]) ? null
                  : undefined

// Hint to specify the underlying object type for data type mapping
const getTypeHint = (val: any) =>
...
const formatType = (name: string, value: any, type: any, typeHint: any, formatOptions: any) => {
  return Object.assign(
...
const formatToTimeStamp = (date: any, treatAsLocalDate: boolean) => {
  const pad = (val: any, num = 2) => '0'.repeat(num - (val + '').length) + val
...
const formatFromTimeStamp = (value: string, treatAsLocalDate: boolean) =>
  !treatAsLocalDate && /^\d{4}-\d{2}-\d{2}(\s\d{2}:\d{2}:\d{2}(\.\d{3})?)?$/.test(value) ?
    new Date(value + 'Z') :
    new Date(value)

interface ExcuteResponse /*extends RDSDataService.ExecuteStatementResponse, RDSDataService.Types.BatchExecuteStatementResponse*/ {
  columnMetadata: any, // ONLY when hydrate or includeResultMetadata is true
  numberOfRecordsUpdated: any, // ONLY for executeStatement method
  records: any, // ONLY for executeStatement method
  generatedFields: any, // ONLY for INSERTS
  updateResults: any // ONLY on batchExecuteStatement
...
const formatRecords = (recs: any, columns: any, hydrate: boolean, formatOptions: FormatOptions | undefined) => {

  if (!recs || !recs[0]) {
    return []
  }
  // Create map for efficient value parsing
  const fmap = recs[0].map((x, i: number) => {
    return Object.assign({},
      columns ? { label: columns[i].label, typeName: columns[i].typeName } : {}) // add column label and typeName
  }) as { label: string, typeName: string, field?: string }[]
    ...
    return rec.reduce((acc, field, i) => {
      ...
      if (field.isNull === true) {
        return hydrate ? // object if hydrate, else array
          Object.assign(acc, { [fmap[i].label as string]: null })
          : (acc as any).concat(null)

        // If the field is mapped, return the mapped field
      } else if (fmap[i] && (fmap)[i].field) {
        const value = formatRecordValue((field as any)[fmap[i].field as string], fmap[i].typeName, formatOptions)
        return hydrate ? // object if hydrate, else array
          Object.assign(acc, { [fmap[i].label as string]: value })
        ...
        Object.keys(field).map(type => {
          if (type !== 'isNull' && (field as any)[type] !== null) {
            (fmap as any)[i]['field'] = type
          }
        })

        // Return the mapped field (this should NEVER be null)
        const value = formatRecordValue((field as any)[(fmap as any)[i].field], fmap[i].typeName, formatOptions)
        return hydrate ? // object if hydrate, else array
          Object.assign(acc, { [(fmap as any)[i].label]: value })
          : (acc as any).concat(value)
      }
...
const formatRecordValue = (value: any, typeName: any, formatOptions: FormatOptions | undefined) => formatOptions && formatOptions.deserializeDate &&
  ...
  : ['BIT'].includes(typeName) ? value == 1 : value

// Format updateResults and extract insertIds
const formatUpdateResults = (res: any) => res.map((x: any) => {
...
// const mergeConfig = (initialConfig: any, args: any) =>
...
// //https://stackoverflow.com/questions/62228176/this-implicitly-has-type-any-because-it-does-not-have-a-type-annotation-ts2
// const query = async function (this: any, config: any, ..._args: any) {
//   // Flatten array if nested arrays (fixes #30)
//   const { isBatch, params, hydrateColumnNames, args, formatOptions } = preSynthesis(_args, config) // end params

//   try { // attempt to run the query

//     // Capture the result for debugging
//     const result = await (isBatch ? config.RDS.batchExecuteStatement(params).promise()
...
//       const rollback = await config.RDS.rollbackTransaction(
...
// const transaction = (config: any, _args: any) => {

//   const args = typeof _args === 'object' ? [_args] : [{}]
//   const queries: (() => any[])[] = [] // keep track of queries
//   let rollback = () => { return; } // default rollback event
...
// const commit = async (config: any, queries: any, rollback: any) => {
...
//     const result: any = await query.apply({ rollback }, [config, queries[i](results[results.length - 1], results)])
...
//     RDS: new RDSDataService(options)
...
const preSynthesis = (_args: any, config: any) => {
  ...
  return { isBatch, params, hydrateColumnNames, args, formatOptions }
}

export {
  preSynthesis, formatResults, ExcuteResponse
}
